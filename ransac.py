...
def RegistrationRANSACBasedOnCorrespondence(
    ...
    if ransac_n < 3 or len(corres) < ransac_n or max_correspondence_distance <= 0.0:
        raise Exception("RegistrationRANSACBasedOnCorrespondence's args were not entered correctly.")
    ...
    for itr in range(criteria.max_iteration):
        if itr < exit_itr_local:
            ...
            check = True
            check = o3d.registration.CorrespondenceCheckerBasedOnEdgeLength(0.9).Check(
                source_p, 
                target_p,
                ransac_corres,
                transformation
            ...
            check = o3d.registration.CorrespondenceCheckerBasedOnNormal(np.pi / 6).Check(
                source_p,
                target_p,
                ransac_corres,
                transformation
            ...
            result = EvaluateRANSACBasedOnCorrespondence(pcd, target_p, corres, max_correspondence_distance, transformation)

            if result.IsBetterRANSACThan(best_result_local):
                best_result_local = result

                # update exit condition if necessary
                exit_itr_d = np.log(1.0 - criteria.confidence) / np.log(1.0 - result.fitness ** ransac_n)
                exit_itr_local = int(np.ceil(exit_itr_d)) if exit_itr_d < criteria.max_iteration else exit_itr_local
